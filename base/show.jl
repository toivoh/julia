...
function show(io::IO, ex::Expr)
    ...
    elseif is(head, :comparison) && nargs >= 3 && (nargs&1==1)  # :comparison
        print(io, '('); io = indent_io(io)
        for arg in args; show(io, arg); end
        io = dedent_io(io); print(io, ')')
    ...
    elseif (nargs == 1 && contains([:return, :abstract, :const], head)) ||
        ...
        @pprint(io, head, ' ', {[show_comma_list](args...)})
...
function show_quoted_expr(io::IO, ex::Expr)
    ...
    else                    @pprint(io, "quote",  '(', {[show](ex)}, ')')
    end
end
show_quoted_expr(io::IO, ex) = @pprint(io, ':', '(', {[show](ex)}, ')')

# ---- end AST prettyprinting -------------------------------------------------
